...
`dirsesh-extras bookmark` maps a character to a directory, the way vim marks do.

```bash
dirsesh-extras bookmark set m ~/code/api      # m is now ~/code/api
dirsesh-extras bookmark get m                 # /home/you/code/api
cd "$(dirsesh-extras bookmark get m)"
```
...
```bash
cd "$(dirsesh-extras bookmark pick)"                  # choose one with fzf
dirsesh at "$(dirsesh-extras bookmark get m)"         # a tmux session at whatever m bookmarks
dirsesh at "$(dirsesh-extras bookmark pick)"          # ...or at one you choose
```
...
```bash
dirsesh-extras bookmark set <char> [path]     # Bookmark a directory (path defaults to the current directory)
dirsesh-extras bookmark remove <char>         # Remove a bookmark
dirsesh-extras bookmark get <char>            # Print the directory a bookmark points at
dirsesh-extras bookmark pick                  # Choose a bookmark with fzf and print its directory
dirsesh-extras bookmark list                  # Every bookmark as "char<TAB>directory"
dirsesh-extras bookmark status [path]         # Bookmarks with a tmux session open at them, for a status line
dirsesh-extras bookmark status-init           # Install the tmux hooks `status` needs (put this in tmux.conf)
```

`dirsesh-extras bookmark -help` prints the same thing.

### Setting and removing

```bash
dirsesh-extras bookmark set m                 # bookmark the current directory at m
dirsesh-extras bookmark set m ~/code/api      # ...or one you name
dirsesh-extras bookmark remove m
```
...
`dirsesh at "$(dirsesh-extras bookmark pick)"` opens nothing when you press escape, rather than
erroring.
...
```tmux
set -g status-right "#(dirsesh-extras bookmark status '#{session_path}')"
```
...
```tmux
set -g status-right "#(dirsesh-extras bookmark status '#{session_path}' -s 'fg=colour244' -c 'fg=black,bg=blue,bold')"
```
...
elsewhere would take that long to appear. `bookmark status-init` installs the two tmux hooks
that make it immediate, on every attached client — put it in `tmux.conf`, where it runs once
per server:

```tmux
run-shell "dirsesh-extras bookmark status-init"
```

It hangs a refresh off `session-created` and `session-closed`, appending to both so anything
else on them survives — `dirsesh init`'s own `session-closed` hook included — and dropping the
hooks a previous run left behind so re-sourcing `tmux.conf` does not stack duplicates. It is
only needed for the status line; nothing else in `bookmark` goes through a hook. Setting and
...
> `set-hook -g`, put `run-shell "dirsesh-extras bookmark status-init"` after it — a later
> `set-hook -g` clears what it appended. The same is true of `run-shell "dirsesh init"`.
...
bind-key \' command-prompt -1 -p "Go to bookmark:" "run-shell -b \"dirsesh at \$(dirsesh-extras bookmark get '%%%')\""
...
```tmux
bind-key m command-prompt -1 -p "Set bookmark:" "run-shell -b \"dirsesh-extras bookmark set '%%%' '#{pane_current_path}'\""
```
...
> shell, so `dirsesh-extras` must be on PATH when that shell starts.
>
...
> `run-shell -b "~/.local/share/tmux-dirsesh/dirsesh-extras bookmark set '%%%'"`.
...
```bash
DIRSESH_BOOKMARKS_DIR=~/.config/work-bookmarks dirsesh-extras bookmark set m ~/work/api
```
